...
<html>
<head>
<title>Stock Management - Ellison Electronics</title>
<meta charset="utf-8">
...
:root {
    --cream:#F7F4EF;--warm:#EDE8E0;--charcoal:#1C1C1E;--mid:#5A5A5A;
    --accent:#3B5BDB;--accent-light:#5C7CFA;--white:#FFFFFF;
    --radius:14px;--radius-lg:20px;--transition:0.3s cubic-bezier(0.25,0.46,0.45,0.94);
    --danger:#E03131;--success:#2F9E44;--warning:#F59F00;
}
...
.prod-table-img { width:44px; height:44px; border-radius:8px; background:#F0F2F5; object-fit:contain; padding:4px; }

/* Stock badges */
.stock-badge { display:inline-flex; padding:4px 10px; border-radius:50px; font-size:11px; font-weight:600; }
.stock-ok { background:rgba(47,158,68,0.1); color:var(--success); }
.stock-low { background:rgba(245,159,0,0.1); color:var(--warning); }
.stock-out { background:rgba(224,49,49,0.1); color:var(--danger); }

.action-btns { display:flex; gap:8px; }
.btn-action { display:inline-flex; align-items:center; gap:4px; padding:6px 14px; border-radius:8px; font-size:12px; font-weight:500; cursor:pointer; transition:all 0.2s; text-decoration:none !important; border:none; font-family:'DM Sans',sans-serif; }
.btn-action-edit { background:rgba(59,91,219,0.08); color:var(--accent); }
.btn-action-edit:hover { background:var(--accent); color:var(--white) !important; }
.btn-action-delete { background:rgba(224,49,49,0.08); color:var(--danger); }
.btn-action-delete:hover { background:var(--danger); color:var(--white) !important; }
...
<body>
    ...
    <aside class="admin-sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="adminStock.jsp" class="active"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 0 1-8 0"/></svg><span>Stock</span></a>
    ...
    <main class="admin-main">
        <div class="admin-topbar">
            <div class="topbar-left"><h1>Stock Management</h1><p>Monitor inventory levels</p></div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        for (ProductBean product : products) {
                            ...
                            int stockQty = product.getProdQuantity();
                            int soldQty = new OrderServiceImpl().countSoldItem(product.getProdId());
                            String stockClass = stockQty > 10 ? "stock-ok" : (stockQty > 0 ? "stock-low" : "stock-out");
                            String stockLabel = stockQty > 10 ? "In Stock" : (stockQty > 0 ? "Low Stock" : "Out of Stock");
                        %>
                        <tr>
                            <td><img src="./ShowImage?pid=<%=product.getProdId()%>" class="prod-table-img" alt=""></td>
                            <td style="font-family:monospace;font-size:12px;color:var(--mid);"><%=product.getProdId()%></td>
                            <td style="font-weight:500;"><%=name%></td>
                            <td style="text-transform:uppercase;font-size:11px;letter-spacing:0.06em;color:var(--mid);"><%=product.getProdType()%></td>
                            <td style="font-weight:600;color:var(--accent);">&#8377; <%=product.getProdPrice()%></td>
                            <td><%=soldQty%></td>
                            <td><%=stockQty%></td>
                            <td><span class="stock-badge <%=stockClass%>"><%=stockLabel%></span></td>
                            <td>
                                <div class="action-btns">
                                    <form method="post" style="display:inline;"><button type="submit" formaction="updateProduct.jsp?prodid=<%=product.getProdId()%>" class="btn-action btn-action-edit">Edit</button></form>
                                    <form method="post" style="display:inline;"><button type="submit" formaction="./RemoveProductSrv?prodid=<%=product.getProdId()%>" class="btn-action btn-action-delete">Remove</button></form>
                                </div>
                            </td>
                        </tr>
                        <% } %>
                        <% if (products.size() == 0) { %>
                        <tr><td colspan="9"><div class="empty-table-state"><p>No items available</p></div></td></tr>
                        <% } %>
                    </tbody>
                </table>
            </div>
