...
<html>
<head>
<title>Pending Orders - Ellison Electronics</title>
<meta charset="utf-8">
...
.status-pending { background:rgba(245,159,0,0.1); color:var(--warning); }
.status-shipped { background:rgba(47,158,68,0.1); color:var(--success); }

.btn-ship { display:inline-flex; align-items:center; gap:5px; padding:7px 16px; border-radius:8px; font-size:12px; font-weight:600; background:var(--success); color:var(--white) !important; text-decoration:none !important; border:none; cursor:pointer; transition:all 0.2s; font-family:'DM Sans',sans-serif; }
.btn-ship:hover { background:#237B32; transform:translateY(-1px); box-shadow:0 4px 12px rgba(47,158,68,0.3); color:var(--white) !important; }
...
<body>
    ...
    <aside class="admin-sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="unshippedItems.jsp" class="active"><svg fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg><span>Pending Orders</span></a>
    ...
    <main class="admin-main">
        <div class="admin-topbar">
            <div class="topbar-left"><h1>Pending Orders</h1><p>Orders awaiting shipment</p></div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="status-badge status-pending">
                                ...
                                Pending</span></td>
                            <td>
                                <a href="ShipmentServlet?orderid=<%=order.getTransactionId()%>&amount=<%=order.getAmount()%>&userid=<%=userId%>&prodid=<%=order.getProductId()%>" class="btn-ship">
                        ...
                        %>
                        <% if (count == 0) { %>
                        <tr><td colspan="7"><div class="empty-table-state"><p>No pending orders</p></div></td></tr>
                        <% } %>
                    </tbody>
                </table>
            </div>
