...
.admin-content{padding:32px 40px;flex:1;display:flex;justify-content:center}
.form-card{background:var(--white);border-radius:var(--radius-lg);box-shadow:0 2px 20px rgba(0,0,0,0.06);padding:44px;width:100%;max-width:720px}
...
.form-group input,.form-group select,.form-group textarea{border:1px solid #E0E0E0;border-radius:10px;padding:12px 16px;font-family:'DM Sans',sans-serif;font-size:14px;color:var(--charcoal);outline:none;transition:border-color 0.3s,box-shadow 0.3s;background:#FAFAFA;width:100%}
.form-group input:focus,.form-group select:focus,.form-group textarea:focus{border-color:var(--accent);box-shadow:0 0 0 3px rgba(59,91,219,0.1);background:var(--white)}
.form-group textarea{min-height:100px;resize:vertical}
.form-actions{display:flex;gap:12px;margin-top:28px;padding-top:20px;border-top:1px solid #EBEBEB}
.btn-submit{flex:1;padding:14px;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:14px;font-weight:600;cursor:pointer;transition:all 0.25s;text-decoration:none!important;text-align:center}
...
.btn-cancel-action{background:#F0F2F5;color:var(--mid);display:flex;align-items:center;justify-content:center}
.btn-cancel-action:hover{background:#E0E0E0;color:var(--charcoal)!important}

/* Product image preview */
.product-preview{display:flex;align-items:center;gap:20px;margin-bottom:24px;padding:16px;background:#FAFAFA;border-radius:12px;border:1px solid #EBEBEB}
.product-preview img{width:80px;height:80px;object-fit:contain;border-radius:8px;background:var(--white);padding:8px}
.product-preview .preview-info{font-size:14px;color:var(--mid)}
.product-preview .preview-info strong{display:block;color:var(--charcoal);font-size:16px;margin-bottom:2px}
...
<body>
    <%
    String utype = (String) session.getAttribute("admin_usertype");
    String uname = (String) session.getAttribute("admin_username");
    String pwd = (String) session.getAttribute("admin_password");
    ...
    <main class="admin-main">
        <div class="admin-topbar">
            <div class="topbar-left"><h1>Update Product</h1><p>Modify product details</p></div>
                ...
                <form action="./UpdateProductSrv" method="post">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="name" value="<%=product.getProdName()%>" required>
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <textarea name="info" required><%=product.getProdInfo()%></textarea>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group">
                            <label>Unit Price (&#8377;)</label>
                            <input type="number" name="price" value="<%=product.getProdPrice()%>" required>
                        </div>
                        <div class="form-group">
                            <label>Stock Quantity</label>
                            <input type="number" name="quantity" value="<%=product.getProdQuantity()%>" required>
                        </div>
                    </div>
                    <div class="form-actions">
                        <a href="adminViewProduct.jsp" class="btn-submit btn-cancel-action">Cancel</a>
                        <button type="submit" class="btn-submit btn-primary-action">Update Product</button>
                    </div>
                </form>
            </div>
        </div>
